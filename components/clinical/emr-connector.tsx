...
export function EmrConnectorPreview() {
  ...
  const connectionSummary = useMemo(() => {
    ...
    return `Connected to a sample EMR feed. ${records.length} records available.`;
  ...
  const loadRecords = useCallback(async function loadRecords() {
    ...
    try {
      ...
      setSelectedIds([]);
      toast.show({ title: "EMR loaded", message: `Loaded ${payload.records?.length ?? 0} sample EMR records.`, type: "success" });
    } catch (fetchError) {
      setRecords([]);
      ...
      <div className="mb-8 grid gap-6 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-violet-50 px-4 py-2 text-sm font-semibold text-violet-900">
            ...
            EMR connector preview
          </div>
          <h1 className="mt-4 text-4xl font-semibold text-violet-950">Direct EMR import workflow</h1>
          <p className="mt-4 max-w-2xl text-base leading-7 text-violet-900/80">
            Review how selected patient summaries can move directly into EndoLab, where they become structured cases with referral readiness and missing-record tasks.
          </p>
        </div>
        ...
        <div className="space-y-3 rounded-3xl border border-violet-100 bg-violet-50 p-5">
          ...
          <div className="flex items-center gap-2 text-base font-semibold text-violet-950">
            ...
            <span>{connected ? "Connected to Sample EMR Feed" : "Connecting..."}</span>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside className="space-y-6 rounded-3xl border border-violet-100 bg-white p-6">
          ...
          <div className="rounded-3xl border border-violet-100 bg-violet-50 p-4 text-sm text-violet-700">
            <p className="font-semibold text-violet-950">Integration note</p>
            <p className="mt-2 leading-6">
              This presentation feed can be swapped for a live FHIR or HL7 extraction connector when a clinic integration is contracted.
            </p>
          </div>
